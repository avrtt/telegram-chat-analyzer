...
COLS_LANG_DICT = {'en': {'date': 'Date', 'week': 'Week', 'month': 'Month', 'timestamp': 'Timestamp',
                  ...
                  'ru': {'date': 'Дата', 'week': 'Неделя', 'month': 'Месяц', 'timestamp': 'Отметка времени',
                         'username': 'Имя пользователя', 'message': 'Сообщение'}}

def get_summarizer_df(filtered_df, language):
    sum_text_col1, _ = st.columns((1000, 0.1))
    with sum_text_col1:
        summrizer_title_lang_dict = {'en': 'Conversations summarizer', 'ru': 'Резюмирование переписок'}
        ...
        date_selector_lng_dict = {'en': 'Select a date', 'ru': 'Выберите дату'}
        month_selector_lng_dict = {'en': 'Select a month', 'ru': 'Выберите месяц'}
        conf_filed_lan_dict = {'en': 'Conversation', 'ru': "Переписка"}
        ...
        with filter_col2:
            conv_selector_lang_dict = {'en': 'Select a conversation', 'ru': "Выберите переписку"}
            all_lang_dict = {'en': 'All', 'ru': "Все"}
            conv = st.selectbox(conv_selector_lang_dict[language],
                                [all_lang_dict[language]] + conv_df_to_sum['Conversations'].to_list())

        with button_col:
            button_lang_dict = {'en': 'Summarize conversations', 'ru': "Резюмировать переписки"}
            st.write('')
            st.write('')
        ...
        orig_text_lang_dict = {'en': "Original conversation", 'ru': "Оригинальная переписка"}
        ...
        with col3:
            sum_text_lang_dict = {'en': "Summarized conversation", 'ru': "Сокращённая переписка"}
...
def get_trends_explorer_widgets(filtered_df, min_date, max_date, language):
    ...
    with global_col:
        explorer_title_lang_dict = {'en': 'Trends analysis', 'ru': "Анализ трендов"}
        st.subheader(explorer_title_lang_dict[language])

        st.divider()

        text_input_lang = {'en': "Filter by text", 'ru': "Фильтровать по тексту"}
        ...
        with col1:
            ...
            with tab_0:
                gran_lang_dict = {'en': ["Monthly", "Weekly", "Daily"], 'ru': ["Ежемесячно", "Еженедельно", "Ежедневно"]}
...
def calc_sentimnets(filtered_df,term, chat_lang, max_words, max_messages, sample_size):
    pattern = f'[^a-zA-Zа-яА-ЯёЁ]{term.lower()}[^a-zA-Zа-яА-ЯёЁ]'
...
def get_sentiment_widget(filtered_df, language, max_words=30, max_messages=300, sample_size=0.2):
    ...
    with global_col:
        ...
        st.divider()
        text_input_lang = {'en': "Select term", 'ru': "Выберите термин"}
